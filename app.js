...
function getGeoLabel(){
  ...
  if(lastPosition && (Date.now() - lastPosition.ts) < MAX_AGE_MS){
    return `${lastPosition.lat.toFixed(5)}, ${lastPosition.lon.toFixed(5)}`;
  }
...
function saveSessions(){
  return idbSet(DB_KEY, sessions).catch((e)=>{
    toast('Aviso: não foi possível salvar agora. Gere o relatório logo para não perder as fotos.', 5000);
    console.error(e);
  });
}
...
function capturePhoto(){
  ...
  const geoLabel = getGeoLabel();
  const padX = Math.round(w*0.03);
  const lineH = Math.round(h*0.028);
  const boxH = lineH*2 + Math.round(h*0.018);
  const boxY = h - boxH - Math.round(h*0.02);
  ctx.font = `${Math.round(h*0.02)}px Arial`;
  const line1 = ts.label;
  const line2 = geoLabel;
  const textW = Math.max(ctx.measureText(line1).width, ctx.measureText(line2).width);
  ctx.fillStyle = 'rgba(11,61,35,0.82)';
  ctx.fillRect(padX - 8, boxY, textW + 16, boxH);
  ctx.fillStyle = '#ffffff';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText(line1, padX, boxY + lineH*0.8);
  ctx.fillText(line2, padX, boxY + lineH*1.8);

  const dataUrl = canvas.toDataURL('image/jpeg', 0.68);

  window._pendingPhoto = { dataUrl, angle, outOfLevel, timeLabel: ts.label, geoLabel };
